...
const purgeSchema = objectType({ what: enumType(["runs", "everything"]) });
const revealSchema = objectType({ path: stringType().min(1) });
const installSchema = objectType({
  what: enumType(["whisper-model", "yt-dlp"]),
...
app.whenReady().then(() => {
  ...
  ipcMain.handle(
    ...
    guard((payload) => {
      ...
      return ask({ type: "install", what, ...model === void 0 ? {} : { model } });
    })
  );
  ...
  ipcMain.handle(
    ...
    guard(async (payload) => {
      ...
      const { response } = await dialog.showMessageBox(window, {
        ...
        detail: everything ? "The database, every extraction, the downloaded speech model and any binary this app installed. Schemas go too. This cannot be undone." : "Every run and its artifacts — frames, transcripts, graphs. Schemas, settings and the downloaded model are kept. This cannot be undone."
      });
      if (response !== 1) return { cancelled: true, freedBytes: 0 };
      const result2 = await ask({ type: "purge", what });
      return { cancelled: false, ...result2 };
    })
  );
  ipcMain.handle(
    CHANNELS.reveal,
    guard(async (payload) => {
      ...
      const { resolvePaths: resolvePaths2 } = await import("./index-DS1cSnpi.js");
      const root = resolvePaths2().data;

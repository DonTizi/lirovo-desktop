...
const Tn = le({
  ...
  description: u().optional(),
  fields: gn(An)
}), Cr = le({ schemaId: u().min(1) }), En = le({ what: Pe(["runs", "everything"]) }), In = le({ path: u().min(1) }), Fn = le({ what: Pe(["whisper-model", "yt-dlp"]) }), Rn = le({ backendId: u().min(1).nullable() });
Pr("kind", [
...
wt.whenReady().then(() => {
  Bn(), Te.handle(be.doctor, Ce(() => Se({ type: "doctor" }))), Te.handle(be.listRuns, Ce(() => Se({ type: "listRuns" }))), Te.handle(
    ...
    be.install,
    Ce((r) => Se({ type: "install", what: Fn.parse(r).what }))
    ...
    be.reveal,
    Ce(async (r) => {
      const { path: e } = In.parse(r), { resolvePaths: t } = await import("./index-v2B9Quaa.js"), n = t().data, a = Me.resolve(e);
